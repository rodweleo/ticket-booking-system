...
export const EventListItem: React.FC<EventProps> = ({ event, index }) => {
    ...
    return <>
        <tr className=" bg-white even:bg-slate-200 hover:bg-slate-300 cursor-pointer">
            <td>{index + 1}</td>
            <td>{event.id}</td>
            <td>{event.title}</td>
            <td>
                <tr className="flex gap-28 w-full">
                    <td>
                        <tr className="flex gap-16 ">
                            <td className="bg-transparent">{event.tickets.types.regular.number} </td>
            ...
            <td className="relative">
                ...
                <ul id={eventActionMenuId} className="absolute hidden bg-slate-400 text-white w-40 top-16 right-0 text-left space-y-1 ">
                    ...
                    <li className="text-red-900 font-bold px-2 py-2 hover:bg-red-300"><button onClick={() => handleDeleteEvent()}><i className="fa-solid fa-trash"></i> Delete Event</button></li>
                </ul>
            </td>
        </tr>
